...
void _PG_init(void);
PG_FUNCTION_INFO_V1(pg_amqp2_exchange_declare);
PG_FUNCTION_INFO_V1(pg_amqp2_publish);
PG_FUNCTION_INFO_V1(pg_amqp2_autonomous_publish);
PG_FUNCTION_INFO_V1(pg_amqp2_disconnect);
...
static void local_amqp2_disconnect_bs(struct brokerstate *);
static void local_amqp2_disconnect(int);
static struct brokerstate *local_amqp2_get_a_bs(int);
static void amqp2_local_phase2(XactEvent, void *);
static struct brokerstate *local_amqp2_get_bs(int);


static void
local_amqp2_disconnect_bs(struct brokerstate *bs) {
	if ( bs && bs->conn ) {
...
static void
local_amqp2_disconnect(int broker_id) {
	struct brokerstate *bs = local_amqp2_get_a_bs(broker_id);
	local_amqp2_disconnect_bs(bs);
}


static void
amqp2_local_phase2(XactEvent event, void *arg)
{
	...
	case XACT_EVENT_COMMIT:
		for ( bs = HEAD_BS; bs; bs = bs->next ) {
			...
			if ( reply.reply_type != AMQP_RESPONSE_NORMAL ) {
				elog(
					WARNING,
					"amqp could not commit tx mode on broker %d, reply_type=%d, library_errno=%d",
					bs->broker_id, reply.reply_type, reply.library_error
				);
				local_amqp2_disconnect_bs(bs);
			}
			bs->uncommitted = 0;
	...
	case XACT_EVENT_ABORT:
		for ( bs = HEAD_BS; bs; bs = bs->next ) {
			...
			if ( reply.reply_type != AMQP_RESPONSE_NORMAL ) {
				elog(
					WARNING,
					"amqp could not rollback tx mode on broker %d, reply_type=%d, library_errno=%d",
					bs->broker_id, reply.reply_type, reply.library_error
				);
				local_amqp2_disconnect_bs(bs);
			}
...
void _PG_init() {
	RegisterXactCallback(amqp2_local_phase2, NULL);
}


static struct brokerstate *
local_amqp2_get_a_bs(int broker_id) {
	struct brokerstate *bs;
	for ( bs = HEAD_BS; bs; bs = bs->next ) {
...
static struct brokerstate *
local_amqp2_get_bs(int broker_id)
{
	char sql[1024];
	char host_copy[300] = "";
	int tries = 0;
	...
	retry:
		...
		if ( SPI_processed > 0 ) {
			...
			bs->sockt = amqp_tcp_socket_new(bs->conn);
			if ( ! bs->sockt ) {
				elog(
					ERROR,
					"amqp2 unable to create socket"
				);
				goto busted;
			}

			status = amqp_socket_open(bs->sockt, host, port);
			if ( status ) {
				elog(
					ERROR,
					"amqp2 unable to open socket: %d",
					status
				);
				goto busted;
			}
...
static Datum
pg_amqp2_publish_opt(PG_FUNCTION_ARGS, int channel) {
	struct brokerstate *bs;
	if ( ! PG_ARGISNULL(0) ) {
		int broker_id;
...
Datum pg_amqp2_exchange_declare(PG_FUNCTION_ARGS)
{
	struct brokerstate *bs;
...
Datum
pg_amqp2_publish(PG_FUNCTION_ARGS)
{
        return pg_amqp2_publish_opt(fcinfo, 2);
}


Datum
pg_amqp2_autonomous_publish(PG_FUNCTION_ARGS)
{
        return pg_amqp2_publish_opt(fcinfo, 1);
}


Datum
pg_amqp2_disconnect(PG_FUNCTION_ARGS)
{
        if ( ! PG_ARGISNULL(0) ) {
                int broker_id = PG_GETARG_INT32(0);
                local_amqp2_disconnect(broker_id);
        }
        PG_RETURN_VOID();
}
